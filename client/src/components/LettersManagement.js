...
class LetterManagement extends Component {
  ...
  componentDidMount = async () => {
    try {
      const {
        status,
        data: { success, leaveLetters: letters, count: page }
      } = await this.props.api();
      if (status !== 200 || !success) return;
      this.setState({ letters, page });
    } catch (err) {
      console.log(err);
    }
  };

  changePage = (page, size) => this.props.api(page, size)
    .then(({ status, data: { success, leaveLetters: letters, count: page } }) => 
      status === 200 && success && this.setState({ letters, page }))
  ...
  render() {
    ...
    const tableInfo = {
      ...
      options: {
        ...
        serverSide: true,
        onTableChange: (action, tableState) => action === 'changePage' && this.changePage(tableState.page, tableState.rowsPerPage),
        onChangeRowsPerPage: size => this.setState({ size })
      }
    };
