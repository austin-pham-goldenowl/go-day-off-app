...
const getDate = (rawDate = '') => {
  ...
  return !date.isValid()
    ? 'Invalid date'
    : dayOfWeek[date.day()] + ', ' + date.locale('vi').format('DD/MM/YYYY');
};
  ...
  render() {
    ...
    const tableInfo = {
      ...
      data: Array.isArray(letters)
        ? letters.map(
            ({ fUserFullName, fFromDT, fToDT, fStatus, fId, fRdt }) => {
              if (type === 'hr')
                return [
                  getDate(fRdt),
      ...
      options: {
        ...
        onTableChange: (action, tableState) =>
          action === 'changePage' &&
          this.changePage(tableState.page + 1, tableState.rowsPerPage)
      }
    };
